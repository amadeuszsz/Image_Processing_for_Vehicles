...
class TrafficSignRecognition():
    ...
    def clear_sign(self, img_hsv, img=None):
        ...
        GPUSetup.program.hsv_bin_mask_center(GPUSetup.queue, (w, h), None, img_buf, mask_buf, np.int32(w), np.int32(h),
                                             dest_buf)
    ...
    def get_labels(self):
        # Getting specifed mask
        lower_bound_r = (165, 90, 70, 0)
        upper_bound_r= (195, 255, 255, 0)
        lower_bound_b = (10, 90, 70, 0)
        upper_bound_b= (25, 255, 255, 0)
        mask_r = self.frame_preprocessing(lower_bound_r, upper_bound_r)
        mask_b = self.frame_preprocessing(lower_bound_b, upper_bound_b)

        label = 1
        # Coordinates (indices [x, y]) of pixels with R channel (BGR code) greater than 40
        coords_r = np.argwhere(mask_r[:, :, 0] > 0)
        coords_b = np.argwhere(mask_b[:, :, 0] > 0)

        labels = np.zeros(shape=(self.height, self.width), dtype=int)
        coords = np.concatenate([coords_r, coords_b])
    ...
    def connected_components(self):
        ...
        #Get initial labels for objects
        labels, coords = self.get_labels()

        #Using Connected Components labeling
        labels = self.ccl_kernel(labels)
    ...
    def templateSumSquare(self, error_limit=15000, old_sign_err_mult=0.9, old_sign_pix_diff=20):
        ...
        for sign in self.signs:
            ...
            for template in self.templates_mask:
                ...
                template_buf = cl.Buffer(GPUSetup.context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
                                         hostbuf=template_masked_flat)
                frame_buf = cl.Buffer(GPUSetup.context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
                                      hostbuf=frame_masked_flat)
                ssd_buf = cl.Buffer(GPUSetup.context, cl.mem_flags.WRITE_ONLY, template_masked_flat.nbytes)
                GPUSetup.program.square_sum(GPUSetup.queue, template_masked_flat.shape, None, template_buf, frame_buf,
                                            ssd_buf)
            ...
            for old_sign in self.old_signs:
                if (old_sign.type is not None):
                    if (np.abs(old_sign.x - sign.x) < old_sign_pix_diff) and (
                            np.abs(old_sign.y - sign.y) < old_sign_pix_diff):
            ...
            if sign.type is not None:
                print("\n+++Sign recognized+++\nType: ", sign.type, " | X: ", sign.x, " | Y: ", sign.y, " | Width: ",
                      sign.width,
                      " | Height: ", sign.height)
        ...
        for sign in self.signs:
            if sign.type is not None:
                template_arr = cv2.resize(self.templates_mask[sign.type], (sign.width, sign.height),
